...
class ResultFrame(wx.Frame):
    ...
    def init_ui(self):
        hbox_headers = wx.BoxSizer(wx.HORIZONTAL)

        title = wx.StaticText(self.panel, label="Node", size=(100, -1))
        font = title.GetFont()
        font.PointSize += 5
        font = font.Bold()
        title.SetFont(font)
        hbox_headers.Add(title, 0, wx.EXPAND | wx.ALIGN_LEFT | wx.ALL, 5)

        t0 = wx.StaticText(self.panel, label="T0", size=(100, -1))
        t0.SetFont(font)
        hbox_headers.Add(t0, 0, wx.EXPAND | wx.ALIGN_CENTER | wx.ALL, 5)

        t1 = wx.StaticText(self.panel, label="T1", size=(20, -1))
        t1.SetFont(font)
        hbox_headers.Add(t1, 0, wx.EXPAND | wx.RIGHT | wx.ALL, 5)

        self.vbox.Add(hbox_headers)

    def show_results(self, Graph):
        nodes_list = list(Graph.nodes.data())
        for node in nodes_list:
            print(node)
            node_name = wx.StaticText(self.panel, -1, "     " + node[0],size=(105,-1))
